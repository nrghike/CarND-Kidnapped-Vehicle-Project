...
void ParticleFilter::init(double x, double y, double theta, double std[]) {
	...
	for(int i = 0; i < num_particles; i++){
		Particle p = {};
		p.id = i;
		// Add normal distributions to each particle
		p.x = dist_x(gen);
		p.y = dist_y(gen);
...
}//end of void ParticleFilter::init
...
}//end of void ParticleFilter::prediction
...
}//end of void ParticleFilter::dataAssociation
...
void ParticleFilter::updateWeights(double sensor_range, double std_landmark[], 
	...
	for(Particle& p : particles){
		...
		// Association of the observation to its nearest landmark
		dataAssociation(predictions, transformed_observations);
		vector<int> associations;
		vector<double> sense_x;
		vector<double> sense_y;

		// Update particle's weight according to observations in Map coordinates from the nearest landmarks
		double weight = 1;
		...
		for(LandmarkObs& new_obs : transformed_observations){
			...
			weight *= prob;
			// Record paticle's associations
			associations.push_back(new_obs.id);
			sense_x.push_back(new_obs.x);
			sense_y.push_back(new_obs.y);
		}

		// Update particle's final weight
		p.weight = weight;
		// Record in weights vector
		weights.push_back(weight);
		// Update particle's associations
		SetAssociations(p, associations, sense_x, sense_y);

	}//end of for(Particle& p : particles)
}//end of void ParticleFilter::updateWeights
...
void ParticleFilter::resample() {
	...
	// Clear particle vectors and record resample particles
	particles.clear();
	weights.clear();
